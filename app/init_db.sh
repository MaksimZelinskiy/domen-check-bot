...
echo "DB_PASSWORD: $DB_PASSWORD"  # Добавим для отладки

# Пробуем подключиться к postgres базе как postgres пользователь
until PGPASSWORD=$DB_PASSWORD psql -h "$DB_HOST" -U postgres -d postgres -c '\l' 2>/dev/null; do
...
PGPASSWORD=$DB_PASSWORD psql -h "$DB_HOST" -U postgres -d postgres -c "DO \$\$
BEGIN
...
PGPASSWORD=$DB_PASSWORD psql -h "$DB_HOST" -U postgres -d postgres -c "ALTER USER $DB_USER WITH SUPERUSER;"

# Проверяем существование базы данных
echo "Checking if database $DB_NAME exists..."
if PGPASSWORD=$DB_PASSWORD psql -h "$DB_HOST" -U postgres -lqt | cut -d \| -f 1 | grep -qw "$DB_NAME"; then
